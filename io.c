...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
    ...
    FILE *lau=NULL;
    char lau_buffer[500];
    //sprintf(lau_buffer,"%s%s_%03d.%d","laudens.dat",All.OutputDir,All.SnapshotFileCount-1,ThisTask);
    sprintf(lau_buffer,"%s%s_%03d.%d",All.OutputDir,"laudens.dat",All.SnapshotFileCount-1,ThisTask);
    ...
    sprintf(lau_buffer,"%s.%d","laudens.dat",ThisTask);
    lau=fopen(lau_buffer,"w");
    for(n = 0; n < pc; pindex++)
    ...
    fprintf(lau,"%d %d %16.18e %16.18lf %16.18lf %16.18lf\n",
    pindex, P[pindex].ID, P[pindex].Density, P[pindex].Pos[0], P[pindex].Pos[1], P[pindex].Pos[2]);
    }
    
    fclose(lau);
    printf("=========================================================\n");
    printf("************we already write laudens file****************\n");
    printf("=========================================================\n");
    */
	  ...
	  printf("I can not open file %s\n",lau_buffer); fflush(stdout);
		  ...
		  for(n = 0; n < pc; pindex++)
			...
			*fp++ = SphP[pindex].Density;
			...
			//-------------this fprintf were commented bynow
			
			fprintf(lau,"%10d %10d %16.8e %16.8lf %16.8lf %16.8lf\n",
				pindex, P[pindex].ID, P[pindex].Density, P[pindex].Pos[0], P[pindex].Pos[1], P[pindex].Pos[2]);
	    ...
	    fclose(lau);
	    printf("=========================================================\n");
	    printf("************we already wrote laudens file****************\n");
	    printf("=========================================================\n");
            break;
...
int blockpresent(enum iofields blocknr)
{
    switch (blocknr)
    {
        ...
        case IO_RAD_ACCEL:
#if defined(FLAG_NOT_IN_PUBLIC_CODE_OUPUT)
            return 1;
#else
            return 0;
#endif
