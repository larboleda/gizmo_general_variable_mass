...
//#include "mass_variation.c"

#include "correct_units_mv.c"
...
void run(void)
{
	...
	/* loop iteration*/
	printf("****************************non standard physics t>0\n*************************************");
	calculate_non_standard_physics();	/* source terms are here treated in a strang-split fashion */
...
void calculate_non_standard_physics(void)  
{
  ...
  double dmdt;                                                                                             
  //dmdt=0.001;
  //double oldmass, mass_t;                                                                                                                                                         
  //oldmass = mass_t = P[i].Mass;                                                                                                     
  //delta_t = tf(bin) -t0(bin)
  double increment;
  //FILE *pf=NULL;
  char nombre[500];
  
  //sprintf(nombre,"loquesea_%d_%f",ThisTask,All.Time);
  //pf = fopen(nombre,"w");
	...
	{ 
	  if(P[i].Mass > 0)
	      ...
	      if(TimeBinActive[P[i].TimeBin])
		{
		  ...
		  tstart = P[i].Ti_begstep + ti_step; //midpoint of step 
		  tend = P[i].Ti_begstep + ti_step;       // end of step
		  
		  delta_t= ti_step * All.Timebase_interval / All.cf_hubble_a; //bintimestep for the active bin                                                                        
		  //increment = dmdt * delta_t; for lvm : dmdt = 0.01

		  dmdt = variation_mass_rate(All.cf_atime);
		  increment = dmdt * delta_t;
		  P[i].Mass = P[i].Mass + increment;
		  
		  //printf("%d %d %d %d %d %lf %d %f %d\n", i, P[i].ID, ti_step, tstart, tend, delta_t, P[i].TimeBin, P[i].Mass, P[i].Ti_current);
		  //fprintf(pf,"%d %d %d %d %d %lf %d %f %d\n", i, P[i].ID, ti_step, tstart, tend, delta_t, P[i].TimeBin, P[i].Mass, P[i].Ti_current);
